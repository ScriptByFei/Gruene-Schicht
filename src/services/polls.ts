...
export async function createPoll(
  ...
  const { data: pollId, error: createError } = await client.rpc('create_poll_with_options', {
...
export async function togglePollOpen(id: string, is_open: boolean): Promise<void> {
  const { error } = await client.from('polls').update({ is_open }).eq('id', id)
  if (error) throw error
}

export async function deletePoll(id: string): Promise<void> {
  const { error } = await client.from('polls').delete().eq('id', id)
  if (error) throw error
}
